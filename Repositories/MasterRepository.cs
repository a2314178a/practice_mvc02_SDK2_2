...
namespace practice_mvc02.Repositories
{
    public class MasterRepository : BaseRepository
    {
        ...
        public object GetThisLvAllAcc(int loginID, bool crossDepart, int loginAccLV, 
            ...
            if(crossDepart)
            {
                if(fDepart == noDepartStr && fPosition == ""){
                ...
                }else if(fDepart == noDepartStr && fPosition != ""){
        ...
        public object GetDepartOption(int loginID, bool cross){
            ...
            if(!cross){
                var query = (from a in _DbContext.accounts
                            ...
                            select new{
                                department=(d==null? noDepartStr:d.department)
                            });
                var myself = (from a in _DbContext.accounts
                            ...
                            where a.ID == loginID
                            select new{
                                department=(c==null? noDepartStr : c.department)
                            });
                        ...
                        if(count == 1)
                        {
                            ...
                            if(emQuery.Count >0)
                            {
                                foreach(var em in emQuery){
                                    ...
                                    _DbContext.employeeprincipals.RemoveRange(emDepartPrincipal);      //刪除舊所屬主管
                                    ...
                                    if(_DbContext.SaveChanges() > 0){
                                        ...
                                        opLog02.content = $"部門職位：{dic["depart"]}{dic["position"]}=>{noDepartStr}";
